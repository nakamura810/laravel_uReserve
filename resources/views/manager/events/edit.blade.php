...
<form method="POST" action="{{ route('events.update', ['event' => $event->id ]) }}">
    ...
    <div>
        ...
        <x-input id="event_name" class="block mt-1 w-full" type="text" name="event_name" value="{{ $event->name }}" required  />
    </div>
...
<div class="flex space-x-4 justify-around">
        <input type="radio" name="is_visible" value="1" @if($event->is_visible === 1){chacked} @endif />表示
        <input type="radio" name="is_visible" value="0" @if($event->is_visible === 0){chacked} @endif />非表示
</div>
<x-button class="ml-4">
            更新する
        </x-button>
